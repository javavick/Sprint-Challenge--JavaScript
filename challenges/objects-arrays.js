...
tyranno.roar = () => "RAWERSRARARWERSARARARRRR!";
...
const universities = graduates.map((atr) => atr.university);
universities.sort();
...
const contactInfo = graduates.map((atr) => `${atr.first_name} ${atr.email}`);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = graduates
  .filter((atr) => atr.university.includes("Uni"))
  .map((atr) => atr.university);
console.log(uni);
...
zooAnimals.forEach((atr) =>
  animalNames.push(
    `Name: ${atr.animal_name}, Scientific: ${atr.scientific_name}`
  )
);
...
const lowerCase = zooAnimals.map((atr) => atr.animal_name.toLowerCase());
...
*/
const lowerPopulation = zooAnimals.filter((atr) => atr.population < 5);
